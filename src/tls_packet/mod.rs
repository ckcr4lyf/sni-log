...
pub fn get_sni(packet: &[u8]) -> Option<&str> {
    ...
    while ext_pos < extension_length as usize {
        let ext_type = u16::from_be_bytes(ethernet_packet.payload[pos + ext_pos .. pos + ext_pos + 2].try_into().expect("Fucked up"));
        ext_pos += 2;
        let ext_length = u16::from_be_bytes(ethernet_packet.payload[pos + ext_pos .. pos + ext_pos + 2].try_into().expect("Fucked up"));
        ext_pos += 2;
        ...
        if ext_type != 0 {
            ...
            if ethernet_packet.payload[pos + ext_pos + sni_pos] == 0x00 {
                sni_pos += 1;
                // DNS hostname
                let hostname_len = u16::from_be_bytes(ethernet_packet.payload[pos + ext_pos + sni_pos .. pos + ext_pos + sni_pos + 2].try_into().expect("Fucked up"));
                sni_pos += 2;
                match std::str::from_utf8(&ethernet_packet.payload[pos + ext_pos + sni_pos .. pos + ext_pos + sni_pos + hostname_len as usize]) {
                    Err(_) => return None,
