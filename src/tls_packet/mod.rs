...
pub fn get_sni(packet: &[u8]) -> Option<&str> {
    ...
    pos += 1;
    println!("session length is {:?}, data is {:?}", session_length, &ethernet_packet.payload[pos .. pos + session_length as usize]);
    ...
    pos += 2;
    println!("Cipher Suite length is {:?}, data is {:?}", cs_length, &ethernet_packet.payload[pos .. pos + cs_length as usize]);
    ...
    pos += 1;
    println!("compression data length is {:?}, data is {:?}", cd_length, &ethernet_packet.payload[pos .. pos + cd_length as usize]);
    ...
    pos += 2;
    println!("extension length is {:?}, data is {:?}", extension_length, &ethernet_packet.payload[pos .. pos + extension_length as usize]);
    // pos += cs_length as usize;
